...
const displayMap = locations => {
  ...
  locations.forEach(loc => {
    ...
    // Add marker
    const marker = L.marker(latlng).addTo(map).bindPopup(`<p>Day ${loc.day}: ${loc.description}</p>`, {
      autoClose: false
    });
    markers.push(marker);
    // Extract coordinates
...
const updateSettings = async (data, type) => {
  try {
    ...
    if (res.data.status === 'success') {
      ...
      window.location.reload(true); // added by me
    }
  } catch (err) {
    (0, _alerts.showAlert)('error', err.response.data.message);
  }
...
const stripe = Stripe('pk_test_51PxbEiEPJE2Ds09UoMkZMALWkCe7OW3tZ1Ka23I1imPej5gUoJYAXyQ4i1kqB8RUfkhmTs3IjGmSROqiX4HHRHvy00shfDwaZU');
